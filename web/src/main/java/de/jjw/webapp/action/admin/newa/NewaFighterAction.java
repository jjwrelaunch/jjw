...
public class NewaFighterAction
    extends BasePage
    implements Serializable, IGlobalWebConstants, INewaFighterConstants
{
    ...
    private static String GOTO_ALL_FIGHTER = "showNewaFighter";

    private static String GOTO_NOT_REG_FIGHTER = "showNotRegistratedNewaFighter";

    private static String GOTO_NEW_FIGHTER = "editNewaFighter";

    private NewaFighterManager fighterManager = null;
    ...
    private NewaFighter fighter = null;
    ...
    public String addFighter()
    {
        ...
        if ( "change".equals( getRequest().getParameter( "main:adminNewaFighterAction:change" ) ) )
        {
            ...
            if ( validRegion )
            {
                ...
                getFacesContext().getExternalContext().getRequestMap().put( "focus", "main:adminNewaFighterAction:team" );
            }
            else
            {
                ...
                getFacesContext().getExternalContext().getRequestMap().put( "focus",
                                                                            "main:adminNewaFighterAction:region" );
            }
        ...
        {
            if ( !NewaFighterValidator.isRequiredFieldsValid( getFighter(), errors )
                && !NewaFighterValidator.isBusinessLogicValid( getFighter(), errors ) )
            {
        ...
        {
            errors.add( new ErrorElement( ADMIN_NEWA_FIGHTER_BLOCKED_POOL ) );
            setErrorMessageVector( errors );
            ret = null;
        }
        catch ( Exception e )
    ...
    public List<Team> getTeams()
    {
        ...
        if ( teams == null || teams.size() == 0 )
        {
            ...
            if ( getRequest().getParameter( "main:adminNewaFighterAction:country" ) != null
                && getRequest().getParameter( "main:adminNewaFighterAction:region" ) != null )
            {
                teams =
                    teamManager.getTeamsByCountryAndRegion(
                                                            Long.valueOf(
                                                                          getRequest().getParameter(
 "main:adminNewaFighterAction:country" ) ).longValue(),
                                                            Long.valueOf(
                                                                          getRequest().getParameter(
 "main:adminNewaFighterAction:region" ) ).longValue() );
            }
    ...
    public NewaFighter getFighter()
    {
        ...
        if ( fighter == null )
        {
            if ( getRequest().getAttribute( WEB_ADMIN_FIGHTER ) != null )
            {
                ...
                fighter = (NewaFighter) getRequest().getAttribute( WEB_ADMIN_FIGHTER );
     ...
     */
    public void setNewaFighter( NewaFighter fighter )
    {
 ...
 "main:adminNewaFighterAction:country" ) ) ) );
            }
