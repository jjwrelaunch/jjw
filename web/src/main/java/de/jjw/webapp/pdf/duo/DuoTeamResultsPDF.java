...
public class DuoTeamResultsPDF
{
    ...
    public DuoTeamResultsPDF( String ressourceBundle, HttpServletResponse response, List<DuoTeamWeb> duoTeamList,
                              Locale locale )
    {
    ...
    public void showResults()
    {
        ...
        {
            log.error( "DuoTeamResultsPDF.showResults: Exception: \ncan not create pdf document \n" + e.getMessage(), e );
        }
    }
    ...
    public ByteArrayOutputStream createContentAsBAOS()
    {
        try
        {
            ...
            for ( int i = 0; i < duoTeamList.size(); )
            {
                ...
                doDuoEntries( i, numberOfRankedFighterInAClass, locale );
        ...
        {
            log.error( "DuoTeamResultsPDF.createContentAsBAOS: Exception: \ncan not create pdf document \n"
                           + e.getMessage(), e );
            return null;
        }
    }
    ...
    /**
     * erstellt die Eintraege pro Klasse und setzt rowIndex weiter
     * 
     * @param startIndex
     * @throws Exception
     */
    private void doDuoEntries( int startIndex, int numberOfFighterEntries, Locale locale )
        throws Exception
    {// write class headline and fighter
        StringBuffer text = new StringBuffer( 30 );
        text.append( duoTeamList.get( startIndex ).getAge().getDescription() );
        ...
        text.append( CodestableMain.getInstance().getCodestableCodeValueByType( ICodestableTypeConstants.TYPE_SEX,
                                                                                duoTeamList.get( startIndex ).getSex(),
                                                                                locale ) );
        text.append( TypeUtil.SPACE );
        text.append( duoTeamList.get( startIndex ).getDuoclass().getDuoclassName() );
        writeText( 50, rowIndex, text.toString(), true, 11, 1 );
        rowIndex -= 15;
        for ( int i = 0; i < numberOfFighterEntries; i++ )
        {
            text = new StringBuffer( 60 );
            text.append( duoTeamList.get( i + startIndex ).getPlace() + ". " );
            text.append( duoTeamList.get( i + startIndex ).getFirstname() ).append( TypeUtil.SPACE );
            text.append( duoTeamList.get( i + startIndex ).getName() );
            text.append( " / " );
            text.append( duoTeamList.get( i + startIndex ).getFirstname2() ).append( TypeUtil.SPACE );
            text.append( duoTeamList.get( i + startIndex ).getName2() );
            writeText( 50, rowIndex, text.toString(), false, 10, 1 );

            writeText( 300, rowIndex, duoTeamList.get( i + startIndex ).getTeam().getTeamName(), false, 10, 1 );
            writeText( 470, rowIndex, duoTeamList.get( i + startIndex ).getTeam().getRegion().getRegionShort(), false,
                       10, 1 );
            writeText( 510, rowIndex, duoTeamList.get( i + startIndex ).getTeam().getCountry().getCountryShort(),
                       false, 10, 1 );
            rowIndex -= 13;
        }
    }
    ...
    private int getNumberOfRankedFighterFromAClass( int startIndex )
    {
        ...
        int ret = 1;
        Long fightingclassId = duoTeamList.get( startIndex ).getDuoclassId();

        // FighterList.add (Fighter[startIndex]);
        startIndex++;
        while ( ( startIndex < duoTeamList.size() )
            && fightingclassId.equals( duoTeamList.get( startIndex ).getDuoclassId() ) )
        {
            startIndex++;
            ret++;
        }
    ...
    private void newPage()
    {
        ...
        {
            log.error( "DuoTeamResultsPDF.newPage: Exception: \ncan not create pdf document \n" + e.getMessage(), e );
        }
    }
        ...
        throws Exception
    {
        ...
        {
            log.error( "DuoTeamResultsPDF.writeText: Exception: \n can not create pdf document \n" + e.getMessage(), e );
        }
        return;
    }
