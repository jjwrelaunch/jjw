    ...
    implements IValueConstants
{
    ...
    private DuoDoublePoolClass duoclass;
    ...
    public DuoDPoolPDF( String ressourceBundle, DuoDoublePoolClass duoclass, HttpServletResponse response,
    ...
    public DuoDPoolPDF( String ressourceBundle, HttpServletResponse response, String headLine, Locale local )
    {
        this.response = response;
        this.rb = ResourceBundle.getBundle( ressourceBundle, local );
        this.headLine = headLine;
        ...
        throws Exception
    {
        try
        {
            ...
            writeText( 600, _540, headLine, BOOLEAN_TRUE, 16 );
            writeText( 600, 510, duoclass.getDuoclassName(), BOOLEAN_TRUE, _14 );
            writeText( _24, _24, rb.getString( "pdf.copyright" ) + "", BOOLEAN_FALSE, 8 );

            writeText( 600, _540, headLine, BOOLEAN_TRUE, 16 );

            createResultTable( _0 );
            fillResultTable( _0, duoclass.getDuoListPoolA() );
            createFightTable( 130, _0 );
            fillFightTable( 130, duoclass.getFightListMapPoolA() );

            createResultTable( 264 );
            fillResultTable( 264, duoclass.getDuoListPoolB() );
            createFightTable( 394, _1 );
            fillFightTable( 394, duoclass.getFightListMapPoolB() );
            ...
            {
                if ( duoclass.getFightListMapPoolA().get( Integer.valueOf( i ) ) != null )
                {
                    createFightList( duoclass.getFightListMapPoolA().get( Integer.valueOf( i ) ), FightNr[i] );
                }

                if ( duoclass.getFightListMapPoolA().get( Integer.valueOf( i + _1 ) ) != null )
                {
                    createFightList( duoclass.getFightListMapPoolA().get( Integer.valueOf( i + _1 ) ), FightNr[i + _1] );
                }

                if ( duoclass.getFightListMapPoolB().get( Integer.valueOf( i ) ) != null )
                {
                    createFightList( duoclass.getFightListMapPoolB().get( Integer.valueOf( i ) ), FightNr[i + 10] );
                }

                if ( duoclass.getFightListMapPoolB().get( Integer.valueOf( i + _1 ) ) != null )
                {
                    createFightList( duoclass.getFightListMapPoolB().get( Integer.valueOf( i + _1 ) ), FightNr[i + 10
                        + _1] );
                }
        ...
        {
            log.error( "DuoPoolDPDF.createPool: Exception: \ncan not create pdf document \n", e );
            throw e;
        }
    }
    ...
    private void fillFightTable( int konst, Map<Integer, DuoFight> fightMap )
        throws Exception
    {
        int height = _18;
        konst += height;
    ...
    private void fillResultTable( int konst, List<DuoDoublePoolItem> duoTeamList )
        throws Exception
    {
        int height = _18, j = _0;
        konst += height;
        for ( int i = _0; i < duoTeamList.size(); i++ )
        {
            ...
            writeText( j + _40, _545 - konst - i * height, duoTeamList.get( i ).getDuoTeam().getName() + " / "
                + duoTeamList.get( i ).getDuoTeam().getName2(), BOOLEAN_FALSE, 6, 1 );
            j += _80;
            writeText( j + _40, _545 - konst - i * height, duoTeamList.get( i ).getDuoTeam().getFirstname() + " / "
                + duoTeamList.get( i ).getDuoTeam().getFirstname2(), BOOLEAN_FALSE, 6, 1 );
            j += _80;
