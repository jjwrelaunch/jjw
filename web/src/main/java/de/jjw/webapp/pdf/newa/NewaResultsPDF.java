...
public class NewaResultsPDF
{
    protected final Logger log = Logger.getRootLogger();
    private ByteArrayOutputStream baos;

    private List<NewaFighterWeb> fighter;
    ...
    public NewaResultsPDF( String ressourceBundle, HttpServletResponse response, List<NewaFighterWeb> fighter,
                               Locale locale )
    {
    ...
    public void showResults()
    {
        ...
        {
            log.error( "NewaResultsPDF.showResults: Exception: \ncan not create pdf document \n" + e.getMessage(),
                       e );
        }
    }
    ...
    {// write class headline and fighter
        ...
        text.append( CodestableMain.getInstance().getCodestableCodeValueByType( ICodestableTypeConstants.TYPE_SEX,
                                                                                ...
                                                                                locale ) );
        text.append( TypeUtil.SPACE );
        text.append( fighter.get( startIndex ).getNewaclass().getWeightclass() );
    ...
    private int getNumberOfRankedFighterFromAClass( int startIndex )
    {
        ...
        int ret = 1;
        Long fightingclassId = fighter.get( startIndex ).getNewaclassId();

        // FighterList.add (Fighter[startIndex]);
        startIndex++;
        while ( ( startIndex < fighter.size() )
 && fightingclassId.equals( fighter.get( startIndex ).getNewaclassId() ) )
        {
            startIndex++;
            ret++;
        }
    ...
    private void newPage()
    {
        ...
        {
            log.error( "NewaResultsPDF.newPage: Exception: \ncan not create pdf document \n" + e.getMessage(), e );
        }
    }
        ...
        throws Exception
    {
        ...
        {
            log.error( "NewaResultsPDF.writeText: Exception: \n can not create pdf document \n"
                           + e.getMessage(), e );
        }
        return;
    }
