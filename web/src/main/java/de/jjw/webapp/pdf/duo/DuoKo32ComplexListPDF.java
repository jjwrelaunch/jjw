 ...
 *
 * File    : DuoKo32ComplexListPDF.java
 * Created : 05 Jun 2010
...
public class DuoKo32ComplexListPDF
    extends DuoKo16ComplexListPDF
{

    public DuoKo32ComplexListPDF( String ressourceBundle, DuoKoClass duoclass, HttpServletResponse response,
                                  String headLine, Locale local )
        throws Exception
    {
        super( ressourceBundle, duoclass, response, headLine, local );

    }

    public DuoKo32ComplexListPDF( String ressourceBundle, HttpServletResponse response, String headLine, Locale local )
    {
        ...
        throws Exception
    {
        try
        {
            ...
            {
                writeTextCentral( 420, 540, headLine, true, _16 );
                writeTextCentral( 420, 510, duoclass.getDuoclassName(), true, 14 );
                ...
                BasePDF.addWatermark( writer.getDirectContentUnder(), "Duo", 140, true );
                manageFightListForKo32( duoclass );
            }
            doc.close();
        ...
        {
            log.error( "DuoKo32ComplexListPDF.createMainRoundTable: Exception: \ncan not create pdf document \n",
                       e );
            throw e;
        }
    }
        ...
        throws Exception
    {
        ...
        {
            writeText( 406, konst - _14 * height + _2, fc.getFinalFight().getDuoTeamRed().getName(), BOOLEAN_FALSE,
                       _10, _0 );
        }
        if ( fc.getFinalFight().getWinnerId().longValue() == fc.getFinalFight().getTeamIdBlue()
            && fc.getFinalFight().getWinnerId().longValue() != TypeUtil.LONG_EMPTY )
        {
            writeText( 434, konst - _16 * height + _2, fc.getFinalFight().getDuoTeamBlue().getName(), BOOLEAN_TRUE,
                       _10, _0 );
        }
     ...
     * @param Ko
     */
    private void manageFightListForKo32( DuoKoClass fc )
    {
        try
        {
            ...
            {
                if ( !( fc.getFightListMapPoolA().get( i ).getTeamIdBlue() == TypeUtil.LONG_MIN ) )
                {
                    createFightList( fc.getFightListMapPoolA().get( i ), count );
                }
                count++;
                if ( !( fc.getFightListMapPoolB().get( i ).getTeamIdBlue() == TypeUtil.LONG_MIN ) )
                {
            ...
            count = manageFightList( fc.getFightListLooserMapPoolA(), _10, _13, count, BOOLEAN_TRUE );
            count = manageFightList( fc.getFightListLooserMapPoolB(), _10, _13, count, BOOLEAN_TRUE );
        ...
        {
            log.error( "DuoKo32ComplexListPDF.manageFightListForKo32: Exception: \ncan not create pdf document \n", e );
        }
    }
