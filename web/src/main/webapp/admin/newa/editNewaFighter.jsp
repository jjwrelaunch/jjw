...
<h:form id="adminNewaFighterAction">
  ...
  <h:inputHidden id="markForExport" value="#{adminNewaFighterAction.fighter.markForExport}"/>
  ...
  <t:panelGrid columns="2">
      ...
      <h:selectOneMenu id="yearOfBirth" value="#{adminNewaFighterAction.fighter.yearOfBirth}" required="false" tabindex="3">
        <f:selectItems value="#{adminNewaFighterAction.yearOfBirthList}"/>
      ...
      <h:outputText value="&nbsp;" escape="false"/>
        ...
        <h:commandButton value="#{msg['edit']}"
                         ...
                         action="#{adminNewaFighterAction.addFighter}" id="addFighter" tabindex="10"/>
        <% } else  {  %>
       
        <h:commandButton value="#{msg['edit']}"
                         onclick="if(myevent !=true) document.getElementById('main:adminNewaFighterAction:change').value= '';"
                         action="#{adminNewaFighterAction.editFighter}" id="addFighter" tabindex="10"/>
        <%} %>
      
      <h:commandButton value="#{msg['abort']}" action="#{adminNewaFighterAction.gotoAllFighter}" id="gotoAllFighter"
                           tabindex="11"/>
    ...
    <%if ( request.getAttribute( IGlobalWebConstants.WEB_ADMIN_FIGHTER ) == null ) {  %>
        ...
        <h:commandButton value="#{msg['editAndNew']}"
                         onclick="if(myevent !=true) document.getElementById('adminNewaFighterAction:change').value= '';"
                         action="#{adminNewaFighterAction.addFighterAndNextTeammember}" id="addFighterAndNextTeammember"
                         tabindex="12"/>
        </t:panelGroup>
    <%}%>
  </t:panelGrid>
