...
<%--
  ...
  ~
  ~ File    : showNewaWeightclasses.jsp
  ~ Created : 17 Jun 2010
  ...
  <script type="text/javascript">
    $( document ).ready( function()
    {
      ...
      highlight( "<h:outputText value="#{adminNewaWeightclassAction.sortColumn}"/>" );
...
<h:form id="adminNewaWeightclassAction">
    <h:commandButton value="#{msg['createNewaFightingList']}" action="#{adminNewaWeightclassAction.gotoCreateNewaFightingList}"
                     id="gotoCreateNewaFightingList" tabindex="2" />
    <h:commandButton value="#{msg['gotoNewWeightclass']}" action="#{adminNewaWeightclassAction.gotoNewWeightclass}"
                     id="gotoNewWeightclass" tabindex="3" />
    <h:commandButton value="#{msg['gotoAutoWeightclass']}" action="#{adminNewaWeightclassAction.gotoAutoWeightclass}"
                     id="gotoAutoWeightclass" tabindex="4" title="#{msg['gotoAutoWeightclass_title']}"/>
    <h:commandButton value="#{msg['sortFighterIn']}" action="#{adminNewaWeightclassAction.sortFighterIn}" id="sortFighterIn"
                     tabindex="5" title="#{msg['sortFighterIn_title']}"/>

  <h:outputText value="#{adminNewaWeightclassAction.text}"/>
  <c:if test="${adminNewaWeightclassAction.ages == null || fn:length(adminNewaWeightclassAction.ages) == 0}">
    <h:outputText value="#{msg.weightclassNoAgeAviable}"/>
  </c:if>

  <c:if test="${ !(adminNewaWeightclassAction.ages == null) && !(fn:length(adminNewaWeightclassAction.ages) == 0) &&
	(adminNewaWeightclassAction.newaWeightclasses != null && fn:length(adminNewaWeightclassAction.newaWeightclasses) > 0)}">
		<f:verbatim>
			<p>&nbsp;</p>
		</f:verbatim>
		...
		<t:dataTable value="#{adminNewaWeightclassAction.newaWeightclasses}" var="cty" border="0" cellspacing="0" cellpadding="0"
                 styleClass="fancyTable" rowClasses="odd,even" width="800px" forceId="true"
                 binding="#{adminNewaWeightclassAction.dataTable}" id="fightingClassTable">
      ...
      <t:column >
        ...
        <h:commandLink action="#{adminNewaWeightclassAction.gotoEditWeightclass}">
          <t:graphicImage value="/images/edit.png" title="#{msg.editTitle}" rendered="true" border="0" alt=""/>        
        </h:commandLink>
        <h:outputText value="&nbsp;" escape="false"/>
        <h:commandLink action="#{adminNewaWeightclassAction.gotoCombineWeightclass}">
          <t:graphicImage value="/images/register_2.png" title="#{msg.combine}" rendered="#{cty.canBeParentClass}" border="0" alt=""/>
        </h:commandLink>
        <h:outputText value="&nbsp;" escape="false"/>
        <h:commandLink action="#{adminNewaWeightclassAction.deleteWeightclass}">
