...
<h:outputText value="#{adminNewaResultsAction.text}"/>

<c:if test="${adminNewaResultsAction.fighters == null ||fn:length(adminNewaResultsAction.fighters) == 0}">
  <h:outputText value="#{msg['results.noresults'] }"/>
</c:if>

<c:if test="${adminNewaResultsAction.fighters != null}">

  <c:if test="${fn:length(adminNewaResultsAction.fighters) > 0}">

    <t:dataTable value="#{adminNewaResultsAction.fighters}" var="cty" border="0" cellspacing="0" cellpadding="0"
                 styleClass="fancyTable" rowClasses="odd,even" width="800px"
                 binding="#{adminNewaResultsAction.dataTable}">
      <t:column>
        <f:facet name="header">
