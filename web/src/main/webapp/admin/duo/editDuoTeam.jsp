...
<h:form id="adminDuoTeamAction">
  ...
  <h:inputHidden id="markForExport" value="#{adminDuoTeamAction.duoTeam.markForExport}"/>
  ...
  <t:panelGrid columns="2">
      ...
      <h:selectOneMenu id="yearOfBirth" value="#{adminDuoTeamAction.duoTeam.yearOfBirth}" required="false" tabindex="3">
        <f:selectItems value="#{adminDuoTeamAction.yearOfBirthList}"/>
      ...
      <h:outputLabel for="yearOfBirth2" value="#{msg['yearOfBirth']}"/>
      <h:selectOneMenu id="yearOfBirth2" value="#{adminDuoTeamAction.duoTeam.yearOfBirth2}" required="false" tabindex="4">
        <f:selectItems value="#{adminDuoTeamAction.yearOfBirthList}"/>
        <jjw:codestableErrorMark field="yearOfBirth"/>
      </h:selectOneMenu>
      ...
      <h:outputText value="&nbsp;" escape="false"/>
        ...
        <% if ( request.getAttribute( IGlobalWebConstants.WEB_ADMIN_DUOTEAM ) == null ) {  %>
        <h:commandButton value="#{msg['edit']}"
                         onclick="if(myevent !=true) document.getElementById('main:adminDuoTeamAction:change').value= '';"
                         action="#{adminDuoTeamAction.addDuoTeam}" id="addDuoTeam" tabindex="11"/>
        <%  }        else     {    %>
       
        <h:commandButton value="#{msg['edit']}"
                         onclick="if(myevent !=true) document.getElementById('main:adminDuoTeamAction:change').value= '';"
                         action="#{adminDuoTeamAction.editDuoTeam}" id="addDuoTeam" tabindex="11"/>
        <%} %>
      
      <h:commandButton value="#{msg['abort']}" action="#{adminDuoTeamAction.gotoAllDuoTeams}" id="gotoAllDuoTeam"
                           tabindex="12"/>     
    ...
    <% if (  request.getAttribute( IGlobalWebConstants.WEB_ADMIN_DUOTEAM ) == null )    {  %>
      ...
      <t:panelGroup colspan="2">     
        <h:commandButton value="#{msg['editAndNew']}"
                         onclick="if(myevent !=true) document.getElementById('adminDuoTeamAction:change').value= '';"
                         action="#{adminDuoTeamAction.addDuoTeamAndNextTeammember}" id="addDuoTeamAndNextTeammember"
                         tabindex="13"/>
      </t:panelGroup>          
    <%}%>
  </t:panelGrid>
