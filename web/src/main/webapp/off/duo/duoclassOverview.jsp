...
<f:verbatim>
  ...
  <script type="text/javascript">
    $( document ).ready( function()
    {
      ...
      highlight( "<h:outputText value="#{tatamiDuoclassOverview.sortColumn}"/>" );
...
<h:form id="tatamiDuoclassOverview">

  <h:outputText value="#{tatamiDuoclassOverview.text}"/>
  <c:if test="${tatamiDuoclassOverview.duoclasses != null}">

    <c:if test="${fn:length(tatamiDuoclassOverview.duoclasses) > 0}">
      <f:verbatim>
        <p>Quick Find: <input type="text" id="quickfind"/> <a id="cleanfilters" href="#">Clear Filters</a></p>
      </f:verbatim>
      <t:dataTable sortColumn="#{tatamiDuoclassOverview.sortColumn}" value="#{tatamiDuoclassOverview.duoclasses}"
                   var="cty" border="0" cellspacing="0" id="duoClassTable" forceId="true"
                   cellpadding="0" styleClass="fancyTable" rowClasses="odd,even" width="800px"
                   binding="#{tatamiDuoclassOverview.dataTable}">
        ...
        <t:column styleClass="center">
          ...
          <h:commandLink action="#{tatamiDuoclassOverview.showDuoclassAsWeb}" rendered="#{cty.classStartet}">
            <t:graphicImage value="/images/webIcon.png"  border="0" alt=""/>
          </h:commandLink>
          <t:graphicImage value="/images/blank.gif" rendered="#{cty.notClassStartet}" border="0" alt="" width="24"/> 
          <h:commandLink action="#{tatamiDuoclassOverview.showDuoclassAsPdf}">
        ...
        <t:column sortable="true">
          ...
          <h:selectBooleanCheckbox onchange="submit()" value="#{cty.classStartet}"
                                   valueChangeListener="#{tatamiDuoclassOverview.startClassListener}"/>
          <h:outputText value="&nbsp;" escape="false"/>
