...
import de.jjw.service.mapper.newa.NewaDoublePoolMapper;
import de.jjw.service.mapper.newa.NewaSimplePoolMapper;
import de.jjw.util.IValueConstants;
import de.jjw.util.TypeUtil;

public class NewaclassPoolsImpl
{
    private static int FIGHT_WIN = 1;

    private static int FIGHT_LOST = 0;

    private static int FIGHT_DIFFERENT_BORDER = 100;

    public NewaSimplePoolClass getNewaclassSimplePool( Long fightingclassId, NewaclassDao newaclassDao,
                                                               int revenge )
        throws JJWManagerException
    {
        try
        {
            NewaSimplePoolClass pool =
                NewaSimplePoolMapper.mapFightingSimplePoolClassFromDB( newaclassDao.getNewaclassSimplePool( fightingclassId ) );
            pool.setId( fightingclassId );
            // compute and fullfill the pool
            NewaclassPoolRankingDTO ranking;
            List<NewaclassPoolRankingDTO> rankingList = new ArrayList<NewaclassPoolRankingDTO>( 5 );
            int i = 0;
            for ( NewaSimplePoolItem item : pool.getFighterList() )
            {
                i++;
                ...
                ranking = new NewaclassPoolRankingDTO();
            ...
            if ( pool.getFighterList().size() > 1 )
            {
                ...
                for ( NewaSimplePoolItem item : pool.getFighterList() )
                {
                    for ( NewaclassPoolRankingDTO dto : rankingList )
                    {
     ...
     * @throws JJWManagerException
     */
    public NewaDoublePoolClass getNewaclassDoublePool( Long fightingclassId, NewaclassDao newaclassDao,
                                                           NewaFightDao fightDao, boolean isResetFinalFight )
        throws JJWManagerException
    {
        try
        {
            NewaDoublePoolClass pool =
                NewaDoublePoolMapper.mapFightingDoubePoolClassFromDB( newaclassDao.getNewaclassDoublePool( fightingclassId ) );
            pool.setId( fightingclassId );
            // compute and fullfill the pool
            NewaclassPoolRankingDTO ranking;
            List<NewaclassPoolRankingDTO> rankingList = new ArrayList<NewaclassPoolRankingDTO>( 5 );
            int i = 0;
            for ( NewaDoublePoolItem item : pool.getFighterListPoolA() )
            {
                i++;
                if ( item.getFight1() != null )
                {
            ...
            // ---------------------------------------------------------------------------------------------
            i = 0;
            rankingList = new ArrayList<NewaclassPoolRankingDTO>( 5 );
            for ( NewaDoublePoolItem item : pool.getFighterListPoolB() )
            {
                i++;
                if ( item.getFight1() != null )
                {
            ...
            for ( NewaDoublePoolItem itemB : pool.getFighterListPoolB() )
            {
                for ( NewaclassPoolRankingDTO dto : rankingList )
                {
            ...
            if ( isAllFightsAreOver( pool.getFightListMapPoolA().values() )
                        ...
                        {
                            NewaFighter fistplacePoolA = getFighterOfRank( 1, pool.getFighterListPoolA() );
                            NewaFighter fistplacePoolB = getFighterOfRank( 1, pool.getFighterListPoolB() );
                            NewaFighter secondplacePoolA = getFighterOfRank( 2, pool.getFighterListPoolA() );
                            NewaFighter secondplacePoolB = getFighterOfRank( 2, pool.getFighterListPoolB() );

                            // halfFinal 1
                            if ( null == fistplacePoolA && null != secondplacePoolB )
                            {
                                NewaFight halfFinalFight = fightDao.getFight( pool.getHalfFinalFight1().getId() );
                                ...
                                pool.getHalfFinalFight1().setFighterRed( new NewaFighter() );
                            ...
                            if ( null != fistplacePoolA && null == secondplacePoolB )
                            {
                                ...
                                pool.getHalfFinalFight1().setFighterBlue( new NewaFighter() );
                            ...
                            {
                                NewaFight halfFinalFight = fightDao.getFight( pool.getHalfFinalFight2().getId() );
                                if ( isResetFinalFight )
                                {
                                    halfFinalFight.resetForKoList();
                                }
                                ...
                                pool.getHalfFinalFight2().setFighterRed( new NewaFighter() );
                            ...
                            if ( null != fistplacePoolB && null == secondplacePoolA )
                            {
                                ...
                                pool.getHalfFinalFight2().setFighterBlue( new NewaFighter() );
                            ...
                            if ( TypeUtil.LONG_EMPTY == pool.getHalfFinalFight2().getWinnerId()
                                ...
                                NewaFight finalFight = fightDao.getFight( pool.getFinalFight().getId() );
                                ...
                                && pool.getHalfFinalFight1().getWinnerId() > TypeUtil.LONG_DEFAULT )
                            {
                                ...
                                pool.getFinalFight().setFighterBlue( new NewaFighter() );
                                ...
                                && pool.getHalfFinalFight1().getWinnerId() == TypeUtil.LONG_DEFAULT )
                            {
                                ...
                                pool.getFinalFight().resetForKoList();
                                pool.getFinalFight().setFighterRed( new NewaFighter() );
    ...
    private void doDoublePoolResults( NewaDoublePoolClass pool )
    {
        // TODO: write Results
        if ( pool.getFinalFight().getWinnerId().longValue() != TypeUtil.LONG_EMPTY )
        {
            ...
            ArrayList<NewaFighter> rdList = new ArrayList<NewaFighter>();
    ...
    private ArrayList<NewaFighter> getAsList(NewaFighter fighter1,NewaFighter fighter2){
        
        ArrayList<NewaFighter> list =new ArrayList<NewaFighter>();
     ...
     * @return
     */
    private static boolean isAllFightsAreOver( Collection<NewaFight> fightList )
    {
        for ( NewaFight fight : fightList )
        {
     ...
     * @return
     */
    private NewaFighter getFighterOfRank( int rank, List<NewaDoublePoolItem> itemList )
    {
        NewaFighter fighter = null;
        for ( NewaDoublePoolItem item : itemList )
        {
     ...
     * @param rankingLis
     */
    private List<NewaclassPoolRankingDTO> doRanking( Collection<NewaFight> fightCollection,
                                                     List<NewaclassPoolRankingDTO> rankingList )
    {
     ...
     */
    private static List<NewaclassPoolRankingDTO> handleDoublePlaces( List<NewaclassPoolRankingDTO> rankingList,
                                                                     Collection<NewaFight> fightCollection,
                                                                         List<Long> placeList, int place )
    {
        for ( NewaFight fight : fightCollection )
        {
     ...
     * @return
     */
    private static List<NewaclassPoolRankingDTO> handle3SamePlaces( List<NewaclassPoolRankingDTO> rankingList,
                                                                    Collection<NewaFight> fightCollection,
                                                                        List<Long> placeList, int place )
    {
        List<NewaFight> fightList = new ArrayList<NewaFight>();
    ...
    private static List<NewaclassPoolRankingDTO> handleWhen1InWinningMap( List<NewaclassPoolRankingDTO> rankingList,
                                                                          int place, List<NewaFight> fightList,
     ...
     * @return
     */
    private static List<NewaclassPoolRankingDTO> handleWhen2InWinningMap( List<NewaclassPoolRankingDTO> rankingList,
                                                                          int place, List<NewaFight> fightList,
                                                                              Map<Long, Integer> fighterWinningMap,
                                                                              List<Long> placeList )
    {
     ...
     * @param fight
     */
    private static boolean isBothFighterInList( List<Long> firstPlaceList, NewaFight fight )
    {
