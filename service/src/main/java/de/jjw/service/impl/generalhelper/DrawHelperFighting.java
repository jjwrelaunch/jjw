...
public class DrawHelperFighting
    extends DrawHelperBase
{
    public List<Fighter> drawFighter( final List<Fighter> fighters, int poolsize, boolean onlyhalfList )
    {
        ...
        for ( int i = 0; i < 5; i++ )
        {
            ...
            if ( checkDoubleFreeRound( ret, poolsize, createCheckPoolHelperDTO( fighters, poolsize ) ) )
                break;
            log.error( "redraw " + fighters.get( 0 ).getFightingclass().getAge().getDescription() + " "
                + fighters.get( 0 ).getFightingclass().getWeightclass() );
            if ( i == 4 )
                log.error( "redraw 4 rounds " + fighters.get( 0 ).getFightingclass().getAge().getDescription() + " "
                    + fighters.get( 0 ).getFightingclass().getWeightclass() );
        }
        return ret;
    }

    private List<Fighter> organizeFighterDraw( final List<Fighter> fighters, boolean onlyhalfList )
    {
        ...
        List<Fighter> retList = new ArrayList<Fighter>( fighters.size() );
        ...
        doDraw( drawList );
        Map<Long, Fighter> hlp = new HashMap<Long, Fighter>();
        for ( Fighter item : fighters )
        {
    ...
    private CheckPoolHelperDTO createCheckPoolHelperDTO( List<Fighter> fighters, int poolsize )
    {
