...
public class FightingclassKoImpl
    extends KoImplAbstract
    implements IValueConstants
{


    public FightingKoClass doKo( FightingKoClass fc, FightDao fightDao )
        throws JJWDataLayerException
    {

        doMainRoundDummyFights( fc );

        List<Fight> changeFights = new ArrayList<Fight>();
...
//				}
        ...
        for ( Fight changedFight : changeFights )
        {
     ...
     * @param fc
     */
    private void doMainRoundDummyFights( FightingKoClass fc )
    {
        // create empty dummy fights, which are not stored in db
        int mainRoundFights = FightingclassKoCreateImpl.getNumberOfFightsInMainRound(
            fc.getFighterListPoolA().size() + fc.getFighterListPoolB().size() ) / 2;
        int poolsize =
            FightingclassKoCreateImpl.getPoolsize( fc.getFighterListPoolA().size() + fc.getFighterListPoolB().size() );
        Fight fight = null;
     ...
     * @return
     */
    private Fight handleFinal( FightingKoClass fc )
    {
     ...
     * @param changeFights
     */
    private void handleLoserRoundA( FightingKoClass fc, List<Fight> changeFights )
    {
        int numberOfFight;
        Fight fight;
        ...
        for (; numberOfFight > 0; numberOfFight-- ) 
        {
            ...
            if ( fight.getWinnerId().longValue() == LONG_DEFAULT  ) //actualFight.getFighterIdBlue()
            {
     ...
     * @param changeFights
     */
    private void handleLoserRoundB( FightingKoClass fc, List<Fight> changeFights )
    {
        int numberOfFight;
        Fight fight;
        //handle solace rounds
    ...
    private Fight updateSolaceRound( FightingKoClass fc, Fight actualFight, int actualFightNumber, boolean isPoolA )
    {

        Fight solaceFight = null;
        int redBlue = 0;
        SolaceDTO dto = null;
     ...
     * this method is for setting the fights, because of the mainround
     *
     * @param fight
     * @param redBlue
     * @param fighter
     */
    private void updateSolaceFight( Fight fight, int redBlue, Fighter fighter )
    {
        long otherFighterId;
        Fighter otherFighter;
     ...
     * @return
     */
    private Fight updateFight( Map<Integer, Fight> fightMap, int numberOfFight, int redBlue, Fighter fighter )
    {
        long otherFighterId;
...
//			updateFight(fightMap, nextFightNumber, (nextFightNumber % 2 == 0)? FIGHTER_RED:FIGHTER_BLUE, null);
     ...
     * @return
     */
    private Fight updateSolaceFight( Map<Integer, Fight> fightMap, int numberOfFight, int redBlue, Fighter fighter )
    {
        long otherFighterId;
        Fighter otherFighter;
        if ( FIGHTER_BLUE == redBlue )
    ...
    private void handle3rdPlaceFights( FightingKoClass fc, List<Fight> changeFights, FightDao fightDao ) throws JJWDataLayerException
    {
        ...
        if(LONG_MIN == fc.getFightListLooserMapPoolA().get( 1 ).getWinnerId() && LONG_MIN != fc.getFightListLooserMapPoolA().get( 0 ).getFighterIdBlue()){
            Fight fight = fc.getFightListLooserMapPoolA().get( 0 );
            long otherFighterId = fight.getFighterIdRed();
            Fighter otherFighter = fight.getFighterRed();
            fight.resetForKoList();
            fight.setFighterRed( otherFighter );
            fight.setFighterIdRed( otherFighterId );
            fight.setFighterBlue( null );           
      ...
      //when fight for 5th place has no winner, so we must ensure that no blue participant for 3rd place fight exists
        if(LONG_MIN == fc.getFightListLooserMapPoolB().get( 1 ).getWinnerId() && LONG_MIN != fc.getFightListLooserMapPoolB().get( 0 ).getFighterIdBlue()){
            Fight fight = fc.getFightListLooserMapPoolB().get( 0 );
            long otherFighterId = fight.getFighterIdRed();
            Fighter otherFighter = fight.getFighterRed();
            fight.resetForKoList();
            fight.setFighterRed( otherFighter );
        ...
        Fighter redFighterA=fc.getFightListLooserMapPoolA().get( 0 ).getFighterRed();
        Fighter redFighterB=fc.getFightListLooserMapPoolB().get( 0 ).getFighterRed();
        ...
        Fight solaceRoundA5thPlaceFight = fc.getFightListLooserMapPoolA().get( Integer.valueOf( 1 ) );
        Fight solaceRoundB5thPlaceFight = fc.getFightListLooserMapPoolB().get( Integer.valueOf( 1 ) );
    ...
    private void handle3rdPlaceFightsWithFreeFight( FightingKoClass fc, List<Fight> changeFights, FightDao fightDao,
                                                    long blueFighterSolaceRoundA, long blueFighterSolaceRoundB,
                                                    long redFighterSolaceRoundA, long redFighterSolaceRoundB, Fighter redFighterA , Fighter redFighterB  )
        throws JJWDataLayerException
    {
            ...
            {// we need to change --> now test if we are allowed to change
                ...
                {   // change
                        ...
                        || ( redFighterSolaceRoundB != LONG_EMPTY && fc.getFightListLooserMapPoolA().get( 0 ).getFighterIdRed() != redFighterSolaceRoundB ) )
                    {
                        ...
                        Fighter otherFighter = fight.getFighterBlue();
                        fight.resetForKoList();
                        fight.setFighterBlue( otherFighter );
                        fight.setFighterIdBlue( otherFighterId );
    ...
    private void handle3rdPlaceFightsWith4Participans( FightingKoClass fc, List<Fight> changeFights, FightDao fightDao,
                                                  long blueFighterSolaceRoundA, long blueFighterSolaceRoundB,
                                                  long redFighterSolaceRoundA, long redFighterSolaceRoundB, Fighter redFighterA , Fighter redFighterB  )
        throws JJWDataLayerException
    {
                                                          ...
                                                          blueFighterSolaceRoundA) )
            {
                ...
                if ( fc.getFightListLooserMapPoolA().get( 0 ).getFighterIdRed() != redFighterSolaceRoundA )
                {
                        ...
                        && fc.getOriginal3rdPlaceFightA().getFighterIdBlue() == otherFighterId )
                    {
                        ...
                        FightMapper.mapFightToDB( fc.getOriginal3rdPlaceFightA(),fight,  false );
                ...
                if ( fc.getFightListLooserMapPoolB().get( 0 ).getFighterIdRed() != redFighterSolaceRoundB )
                {
                    ...
                    {
                        // test, if we use the fight from database or reset it                        
                        FightMapper.mapFightToDB( fc.getOriginal3rdPlaceFightB(), fight, false );
    ...
    private List<Fight> removeChangedFightFromList( long fightId, List<Fight> changeFights )
    {
        int index = 0;
